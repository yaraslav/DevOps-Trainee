...
При попытке перейти на сайт по IP-адресу можем получить информацию о сертификате ( о владельце, имя и т.д.):
```bash
...
```
В web-браузере возможно вообще разрешить и перейти в незащещенное подключение.
Поэтому чтобы это исправить и запретить доступ по ip-адресу добавляем в конфиг `default servers`:
```bash
server {
